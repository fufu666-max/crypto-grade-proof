  ...
  useEffect(() => {
    ...
    if (_isRunning === true) {
      ...
      createFhevmInstance({
        ...
        .catch((e) => {
          console.log(`[useFhevm] Error creating FHEVM instance:`, e);
          if (thisSignal.aborted) {
            console.log(`[useFhevm] Aborted, ignoring error`);
            return;
          }
          ...
          // Check if it's a network error (common in local development)
          const errorMessage = e?.message || String(e);
          if (errorMessage.includes("Failed to fetch") || 
              errorMessage.includes("ERR_CONNECTION_CLOSED") ||
              errorMessage.includes("AbortError") ||
              e?.name === "FhevmAbortError") {
            // These are expected in local development when external services are unavailable
            console.warn(`[useFhevm] Network error (expected in local dev):`, errorMessage);
            // Don't set error status for network issues - let it retry
            _setInstance(undefined);
            _setError(undefined);
            _setStatus("idle");
            return;
